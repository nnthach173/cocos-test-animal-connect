...
export default class LineGame extends cc.Component
{
    ...
    onLoad() {
        ...
        //     this.stepLabel.string = UserInfo.stepUsed + " " + LocalizationManager.t("current_step");
        // },1)
        ...
        this.schedule(()=>{
            ...
            let timeLimit = 10 * UserInfo.currentLevel;
            if (UserInfo.timePassed >= timeLimit) {
                this.onTimeout();
            }
        }, 1);
    }
        ...
        if (!t._isGameOver) {
            ...
            if( t.checkConnectedAll())
            {
                if(t.checkFillAll())
                {
                    ...
                    Toast.make(LocalizationManager.t("fill_all"));
                }
            }else{
                // _uiManager.hideFillAllPopup()
            }
    ...
    onTimeout() {
        ...
        ViewManager.instance.show("Game/TimeOutDialog")
}
}